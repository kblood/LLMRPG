...
class OllamaRPGApp {
  ...
  /**
   * Handle unified state updates from StatePublisher
   */
  handleStateUpdate(state, eventType) {
    console.log('[App] Handling state update:', eventType, 'Frame:', state.frame);
    console.log('[App] State structure:', {
      hasTime: !!state.time,
      hasCharacters: !!state.characters,
      hasProtagonist: !!(state.characters?.protagonist),
      hasQuests: !!state.quests,
      hasQuestsActive: !!(state.quests?.active),
      questsCount: state.quests?.active?.length || 0,
    ...
    switch (eventType) {
      ...
      case 'action_executed':
        if (state.lastAction) {
          this.handleActionExecuted(state.lastAction);
        }
        break;
      ...
      case 'dialogue_line':
        if (state.lastDialogue) {
          this.addMessageToHistory({
            speakerName: state.lastDialogue.speakerName,
            speaker: state.lastDialogue.speakerId,
            text: state.lastDialogue.text
          });
        }
        break;
      ...
      case 'combat_started':
        if (state.activeCombat) {
          this.handleCombatStarted(state.activeCombat);
        }
        break;
        
      case 'combat_ended':
        if (state.lastCombatResult) {
          this.handleCombatEnded(state.lastCombatResult);
        }
        break;
      ...
      case 'location_changed':
        // Already handled by updateLocationsDisplay above
        if (state.characters && state.characters.protagonist && state.characters.protagonist.currentLocation) {
          const location = state.location.database?.find(l => l.id === state.characters.protagonist.currentLocation);
          const locationName = location ? location.name : state.characters.protagonist.currentLocation;
          this.addEventToLog(`📍 At ${locationName}`, 'location-info');
        }
        break;
    }
  }
  ...
  /**
   * Handle game events broadcast from StatePublisher
   */
  handleGameEvent(event) {
    console.log('[App] Handling game event:', event.type);
    // Handle broadcast events as needed
  }
  
  /**
   * Update time display from state
   */
  updateTimeDisplay(time) {
    if (!time) return;
    
    // Parse game time if it's a number (minutes since start)
    let hours, minutes;
    if (typeof time.gameTime === 'number') {
      hours = Math.floor(time.gameTime / 60) % 24;
      minutes = Math.floor(time.gameTime % 60);
    ...
    const period = time.timeOfDay || this.getPeriodOfDay(hours);
  ...
  /**
   * Update protagonist display from state
   */
  updateProtagonistDisplay(protagonist) {
    if (!protagonist) return;
    
    // Update name and basic info
    const nameEl = document.getElementById('player-name');
    if (nameEl && protagonist.name) {
      nameEl.textContent = protagonist.name;
    }
    ...
    if (protagonist.stats) {
      ...
      // Stamina
      if (stats.currentStamina !== undefined && stats.maxStamina !== undefined) {
        const staminaPercent = (stats.currentStamina / stats.maxStamina) * 100;
        document.getElementById('stamina-text').textContent = `${stats.currentStamina}/${stats.maxStamina}`;
        document.getElementById('stamina-fill').style.width = `${staminaPercent}%`;
      }
      ...
      // Magic
      if (stats.currentMagic !== undefined && stats.maxMagic !== undefined) {
        const magicPercent = (stats.currentMagic / stats.maxMagic) * 100;
        document.getElementById('magic-text').textContent = `${stats.currentMagic}/${stats.maxMagic}`;
        document.getElementById('magic-fill').style.width = `${magicPercent}%`;
      }
      ...
      // XP and Level
      if (stats.currentXP !== undefined && stats.xpToNextLevel !== undefined) {
        const xpPercent = (stats.currentXP / stats.xpToNextLevel) * 100;
        document.getElementById('xp-text').textContent = `${stats.currentXP}/${stats.xpToNextLevel}`;
        document.getElementById('xp-fill').style.width = `${xpPercent}%`;
      }
    ...
    // Update gold
    if (protagonist.inventory && protagonist.inventory.gold !== undefined) {
      document.getElementById('player-gold').textContent = protagonist.inventory.gold;
    }
  }
  ...
  updateQuestsDisplay(quests) {
    ...
    quests.forEach(quest => {
      console.log('[App] Checking quest for newness:', quest.title, 'isNew:', quest.isNew);
      // Always log quests that were just created (they may not have isNew flag)
      if (quest.isNew || !this._loggedQuests) {
        if (!this._loggedQuests) this._loggedQuests = new Set();
        if (!this._loggedQuests.has(quest.id)) {
          this.addEventToLog(`📜 New Quest: ${quest.title}`, 'quest-new');
          this._loggedQuests.add(quest.id);
        }
      }
    });
  }
  ...
  /**
   * Update locations display from state
   */
  updateLocationsDisplay(locationState) {
    ...
    console.log('[App] Discovered locations:', discoveredCount, 'element:', locationCountEl);
    if (locationCountEl) {
        ...
        worldLocationsEl.innerHTML = discoveredData.map(loc => {
          ...
          return `
            <div class="location-item ${isCurrent ? 'current-location' : ''} ${isVisited ? 'visited' : 'unvisited'}">
              ...
              <div class="location-danger">${loc.dangerLevel || 'safe'}</div>
            </div>
          `;
        }).join('');
        console.log('[App] World locations updated with HTML');
      } else {
        worldLocationsEl.innerHTML = '<p class="no-locations">Exploring...</p>';
      }
    }
  }
    ...
    switch (action.type) {
      case 'travel':
        if (action.from && action.to) {
          message = `🛤️ Traveled from ${action.from} to ${action.to}`;
        }
        break;
      case 'rest':
        ...
        break;
      case 'explore':
        message = `🔍 Explored the area`;
        break;
      case 'talk':
        message = `💬 Started conversation with ${action.targetName || 'someone'}`;
        break;
    }
    
    if (message) {
      this.addEventToLog(message, 'action-result');
    }
    
    if (action.narrative) {
      document.getElementById('gm-narration').textContent = action.narrative;
    }
  }
  ...
  handleCombatStarted(combat) {
    ...
    const enemies = combat.enemies?.map(e => e.name).join(', ') || 'Unknown enemies';
    const message = `⚔️ Combat! Encountered ${enemies}`;
    
    document.getElementById('gm-narration').textContent = message;
    this.addEventToLog(message, 'combat-encounter');
   ...
   */
  handleCombatEnded(result) {
    if (!result) return;
    
    const narrative = result.narrative || `Combat ended in ${result.outcome}!`;
    document.getElementById('gm-narration').textContent = narrative;
    this.addEventToLog(`⚔️ ${narrative}`, 'combat-result');
    
    if (result.xpGained) {
      const rewardMsg = `Combat victory! Gained ${result.xpGained} XP${result.goldGained ? ` and ${result.goldGained} gold` : ''}`;
      this.addEventToLog(`💰 ${rewardMsg}`, 'reward');
      this.setStatus(rewardMsg);
    } else if (result.outcome === 'defeat' || result.outcome === 'timeout') {
      this.setStatus(`Combat ${result.outcome}. Lost ${result.goldLost || 0} gold.`);
    }
  }
  
  /**
   * Get period of day from hours
   */
  getPeriodOfDay(hours) {
  ...
  setupAutonomousListeners() {
    ...
    // ============================================================================
    // NEW UNIFIED STATE UPDATE LISTENER (StatePublisher integration)
    // ============================================================================
    this.gameAPI.onGameUpdate((update) => {
      updateCount++;
