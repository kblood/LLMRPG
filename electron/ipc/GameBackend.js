...
export class GameBackend {
  ...
  async initialize(options = {}) {
    ...
    try {
      ...
      let npcsObject;
      if (options.worldConfig && options.worldConfig.npcs && options.worldConfig.npcs.length > 0) {
        // Use NPCs from world config (generated by main menu)
        console.log('[GameBackend] Using generated NPCs from world config');
        npcsObject = {};
        options.worldConfig.npcs.forEach((npcData, idx) => {
          const npcId = `npc_${idx}`;
          npcsObject[npcId] = new Character(npcId, npcData.name, {
        ...
        // Fall back to default NPCs
        console.log('[GameBackend] Using default pregenerated NPCs');
        npcsObject = createAllNPCs();
      }
      ...
      // Store world config and main quest if provided
      if (options.worldConfig) {
        this.worldConfig = options.worldConfig;
        console.log('[GameBackend] Stored world config from main menu');

        // Set main quest if available
        if (options.worldConfig.quests && options.worldConfig.quests.main) {
          this.mainQuest = options.worldConfig.quests.main;
          console.log('[GameBackend] Set main quest from world config');
        }
      }
